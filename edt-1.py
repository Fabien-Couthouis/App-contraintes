...
cours['IT101'] = {'respo': 'db', 'amphi': 10, 'td': 20, 'tp': 0}
...
# Creation of dependances
dependance = {}
dependance['IF102'] = ['IF101']
dependance['IS102'] = ['IS101']
dependance['IT102'] = ['IT101']

# 6 slots par jour, 5 fois par semaine, sur 10 semaines
slots = list(range(0, 6*5*10))
students = list(range(0, 100))  # 100 students
rooms = list(range(0, 5))  # 5 salles de cours
# A remplir : les respos sont compétents dans leur matiere
...
weeks = [list(range(i*slotsByWeek, (i+1)*slotsByWeek))
         for i in range(nSemesterWeeks)]
...
satisfy(
    ...
    [imply(s[i] == s[j], r[i] != r[j]) for i in range(nLectures)
     for j in range(i + 1, nLectures)],

    # no two lectures with the same teacher at the same time
    [imply(s[i] == s[j], t[i] != t[j]) for i in range(nLectures)
     for j in range(i + 1, nLectures)],

    # no two lectures with the same group at the same time
    [imply(s[i] == s[j], g[i] != g[j]) for i in range(nLectures)
     for j in range(i + 1, nLectures)],

    # Ensures that the room has a sufficient capacity for students
    [imply(r[i] == room, sr[room] > sl[i])
     for i in range(nLectures) for room in range(nRooms)],

    # Channeling constraint
    [imply((t[i] == ti) & (s[i] == si), ts[ti][si]) for i in range(nLectures)
     for ti in range(nTeachers) for si in range(nSlots)],

    # Teacher does not teach too much per week
    # [Sum(ts[t][s] for s in week) <=
    #  maxTeachingPerWeek for week in weeks for t in range(nTeachers)],

    # [Count((t[i] == ti) & (s[i] == si) for si in week) <=
    #  maxTeachingPerWeek for week in weeks for i in range(nLectures) for ti in range(nTeachers)],


    # [Count((t[i] == ti) & (s[i] == si) for si in week) <= maxTeachingPerWeek for week in [
    #     [1, 2]] for i in range(10, 12) for ti in range(20, 22)],
